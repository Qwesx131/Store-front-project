    ...
    switch (category) {
      case "fragrances":
        categoryProducts = allProducts.filter(
          (p) => p.category.toLowerCase() === "fragrances"
        );
        break;

      case "home-decoration":
        categoryProducts = allProducts.filter(
          (p) => p.category.toLowerCase() === "home-decoration"
        );
        break;
      ...
      case "groceries":
        categoryProducts = allProducts.filter(
          (p) => p.category.toLowerCase() === "groceries"
        );
        break;

      case "homepage":
        categoryProducts = allProducts.sort(() => 0.5 - Math.random()).slice(0, 6);
        break;
